...
const ButtonLongevityEmployees = () => {
    const [showModal, setShowModal] = useState(false);
    const [selectedYear, setSelectedYear] = useState('');
    const [queryResponse, setQueryResponse] = useState([]);
    const [errorMessage, setErrorMessage] = useState('');
    const { authData } = useAuth();
    const urlData = `${process.env.SV_HOST}${process.env.SV_PORT}${process.env.SV_ADDRESS}${process.env.RALL_SUMMARY_LONGEVITY_EMPLOYEES}`;
    ...
    const generatePDF = () => {
        ...
        doc.setFontSize(12);
        doc.text(`Rango de Edad entre los empleados`, 10, 20); // x, y

        doc.setFontSize(8);
        doc.text("Este es un informe de los rangos etarios en la organización.", 10, 40);
        doc.text("Este informe muestra distintos rangos etarios y a su par la cantidad de empleados pertenecientes", 10, 50);
    ...
    return (
        <div className='report__container'>
            ...
            <div className='report__title__container'>
                ...
                <button onClick={toggleModal} className='report__button__title'>Rango de Edad de Empleados</button>
            </div>
            {showModal && (
...
export default ButtonLongevityEmployees;
