...
const ButtonDiversityNacionalities = () => {
    const [showModal, setShowModal] = useState(false);
    const [selectedYear, setSelectedYear] = useState('');
    const [queryResponse, setQueryResponse] = useState([]);
    const [errorMessage, setErrorMessage] = useState('');
    const { authData } = useAuth();
    const urlData = `${process.env.SV_HOST}${process.env.SV_PORT}${process.env.SV_ADDRESS}${process.env.RALL_SUMMARY_DIVERSITY_EMPLOYEES}`;
    ...
    const generatePDF = () => {
        ...
        doc.setFontSize(12);
        doc.text(`Nacionalidad de los empleados`, 10, 20); // x, y

        doc.setFontSize(8);
        doc.text("Este es un informe geografico de las distintas nacionalidades en la organización.", 10, 40);
        doc.text("Este informe muestra las distintas nacionalidades y a su par la cantidad de empleados pertenecientes", 10, 50);
        doc.text(`Reporte generado el: ${day}/${month}/${year} a las ${hour}:${minutes}`, 10, 60)

        doc.setFontSize(10);
        doc.text(`Total de Empleados: ${queryResponse[0].total_employees}`, 10, 70)


        doc.autoTable({
            head: [['Nacionalidad', 'Cantidad de Empleados', "Porcentaje"]],
            body: queryResponse.map(item => [
                item.name_nacionality,
                item.total_nacionalities,
        ...
        doc.save(`Diversidad_De_Nacionalidades.pdf`);
    };
    ...
    return (
        <div className='report__container'>
            ...
            <div className='report__title__container'>
                ...
                <button onClick={toggleModal} className='report__button__title'>Diversidad de Nacionalidades</button>
            </div>
            {showModal && (
...
export default ButtonDiversityNacionalities;
