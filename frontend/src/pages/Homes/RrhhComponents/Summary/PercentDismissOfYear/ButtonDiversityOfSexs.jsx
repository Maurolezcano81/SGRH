...
const ButtonDiversityOfSexs = () => {
    const [showModal, setShowModal] = useState(false);
    const [selectedYear, setSelectedYear] = useState('');
    const [queryResponse, setQueryResponse] = useState([]);
    const [errorMessage, setErrorMessage] = useState('');
    const { authData } = useAuth();
    const urlData = `${process.env.SV_HOST}${process.env.SV_PORT}${process.env.SV_ADDRESS}${process.env.RALL_SUMMARY_DIVERSITY_OF_SEXS}`;
    ...
    const generatePDF = () => {
        ...
        doc.setFontSize(12);
        doc.text(`Diversidad de Sexos entre los empleados`, 10, 20); // x, y

        doc.setFontSize(8);
        doc.text("Este es un informe de la diversidad de sexos existentes en la organización.", 10, 40);
        doc.text("Este informe muestra la cantidad de tipos de sexos y sus cantidades", 10, 50);
        ...
        doc.autoTable({
            head: [['Tipo de Sexo', 'Cantidad de Empleados', 'Porcentaje']],
        ...
        doc.save(`Diversidad_De_Sexos.pdf`);
    };
    ...
    return (
        <div className='report__container'>
            ...
            <div className='report__title__container'>
                ...
                <button onClick={toggleModal} className='report__button__title'>Diversidad de Sexos</button>
            </div>
            {showModal && (
...
export default ButtonDiversityOfSexs;
