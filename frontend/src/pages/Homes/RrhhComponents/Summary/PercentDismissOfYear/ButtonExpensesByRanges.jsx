...
const ButtonExpensesByRanges = () => {
    const [showModal, setShowModal] = useState(false);
    const [selectedYear, setSelectedYear] = useState('');
    const [queryResponse, setQueryResponse] = useState([]);
    const [errorMessage, setErrorMessage] = useState('');
    const { authData } = useAuth();
    const urlData = `${process.env.SV_HOST}${process.env.SV_PORT}${process.env.SV_ADDRESS}${process.env.RALL_SUMMARY_EXPENSES_BY_RANGES}`;
    ...
    const generatePDF = () => {
        ...
        doc.setFontSize(12);
        doc.text(`Cantidad de Empleados Por Rango Salarial`, 10, 20); // x, y

        doc.setFontSize(8);
        doc.text("Este es un informe donde se muestran la cantidad de Empleados según un rango salarial", 10, 40);
        doc.text("Se muestra el rango de salario, el total de empleados en cada rango, y el porcentaje correspondiente.", 10, 50);
        doc.text(`Reporte generado el: ${day}/${month}/${year} a las ${hour}:${minutes}`, 10, 60)

        doc.autoTable({
            head: [['Rango Salarial', 'Cantidad De Empleados', "Porcentaje"]],
        ...
        doc.save(`Cantidad_De_Empleados_Por_Rangos_Salariales.pdf`);
    };
    ...
    return (
        <div className='report__container'>
            ...
            <div className='report__title__container'>
                ...
                <button onClick={toggleModal} className='report__button__title'>Empleados Por Rangos Salariales</button>
            </div>
            {showModal && (
...
export default ButtonExpensesByRanges;
